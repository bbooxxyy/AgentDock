...
const rows = [
  ...
  ,["appUpdate.currentVersion", "目前版本", "Current version", "現在のバージョン", "Aktuelle Version"]
  ,["appUpdate.checkLatest", "檢測最新版", "Check latest version", "最新版を確認", "Neueste Version prüfen"]
  ,["appUpdate.checking", "正在檢測", "Checking", "確認中", "Wird geprüft"]
  ,["appUpdate.checkHint", "檢測 AgentDock 最新版本", "Check for the latest AgentDock version", "AgentDock の最新版を確認", "Neueste AgentDock-Version prüfen"]
  ,["appUpdate.latest", "目前已是最新版", "AgentDock is up to date", "現在は最新版です", "AgentDock ist aktuell"]
  ,["appUpdate.available", "發現新版本 {version}", "Version {version} is available", "新しいバージョン {version} があります", "Version {version} ist verfügbar"]
  ,["appUpdate.unsupported", "目前平台不支援自動更新", "Automatic updates are not supported on this platform", "このプラットフォームでは自動更新に対応していません", "Automatische Updates werden auf dieser Plattform nicht unterstützt"]
  ...
  ,["telemetry.reportIssue", "上報異常", "Report issue", "問題を報告", "Problem melden"]
  ,["telemetry.reportIssueHint", "上報全部執行日誌以供異常分析", "Upload all execution logs for issue analysis", "問題分析のためにすべての実行ログを送信", "Alle Ausführungsprotokolle zur Problemanalyse hochladen"]
  ,["telemetry.reportQueued", "已提交 {count} 條執行日誌，正在背景上報", "Submitted {count} execution logs for background upload", "{count} 件の実行ログを送信し、バックグラウンドでアップロードしています", "{count} Ausführungsprotokolle wurden zum Hintergrund-Upload eingereicht"]
  ,["telemetry.reportCurrent", "執行日誌均已上報，異常記錄已提交檢查", "Execution logs are up to date; exception records were submitted for review", "実行ログはすべて送信済みです。例外記録の確認を開始しました", "Ausführungsprotokolle sind aktuell; Ausnahmedaten wurden zur Prüfung eingereicht"]
  ,["telemetry.notConfigured", "遙測上報設定尚未寫入安裝包，執行日誌已保留", "Telemetry upload is not configured in this build; execution logs were retained", "このビルドにはテレメトリアップロード設定がありません。実行ログは保持されました", "Der Telemetrie-Upload ist in diesem Build nicht konfiguriert; Ausführungsprotokolle wurden beibehalten"]
